 ...
 * @Last Modified time: 2018-12-04 16:02:42
 */
...
class Order extends ShopAdminAuth
{
    ...
    public function index(Request $request){
        ...
        try{
            ...
            {
                $orderBy['filter']['orderBy'] = 'bo.deal_time';
            }
            ...
            $order_list = $orderModel
                            ...
                            ->field('bog.gid,bog.cat_id,bog.status bog_status,bog.goods_name,bog.goods_sketch,bog.goods_original_price,bog.goods_price,bog.verify_time')
            ...
            if ($order_list) {
                ...
                //计算总金额
                $total_amount = $orderModel
                                ->alias('bo')
                                ->join('dts_bill_order_goods bog','bo.oid = bog.oid', 'LEFT')
                                ->join('dts_bill_order_reserve bor','bog.rid = bor.rid','LEFT')
                                ->field("sum(bo.deal_amount) as total_amount")
                                ->where($where)
                                ->where("bo.sid", $sid)
                                ->find();
                ...
                $res = [
                    ...
                    'data'         => $order_list,
                    'total_amount' => sprintf("%.".$decimal."f", $total_amount['total_amount'])
                ];
    ...
    public function orderStatus(){
        $status_list = config("order.order_status");
